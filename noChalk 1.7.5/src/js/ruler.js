...
function placeSetSqInView() {
  const u = sqCm();
  const { B, H } = SQ;
  const pad = 90 / VP.z; /* room for the handles */
  setSq.x = -VP.x / VP.z + B * u + pad;
  setSq.y = -VP.y / VP.z + H * u + pad;
  if (state && state.ui) state.ui.setsqPos = { x: setSq.x, y: setSq.y, rot: setSq.rot };
}
