...
async function importFile(f) {
  ...
  if (ext === "html" || ext === "htm") return importHtmlFile(f);
...
async function importHtmlFile(f) {
  const t = progToast("Reading " + f.name);
  try {
    const text = await f.text();
    if (!text.trim()) throw new Error("empty file");
    insertRunHtml(text);
    t.done(f.name + " — inserted as a live HTML box");
  } catch (e) {
    t.fail("HTML import failed: " + (e.message || "error"));
  }
}

function insertEmbedHTML(html, fit) {
  const wx = Math.max(24, (vpW / 2 - VP.x) / VP.z - DEFMEDIAW / 2),
    wy = Math.max(pgHead.offsetHeight, (vpH / 2 - VP.y) / VP.z - 60);
  const rec = { id: uid(), x: snapVal(wx), y: snapVal(wy), w: DEFMEDIAW, ro: true, html: html, fit: fit ? 1 : undefined };
