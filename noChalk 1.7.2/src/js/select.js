...
/* the marquee/lasso waits for real movement before it appears:
   a touch down alone starts nothing, so two-finger navigation never
   flashes or drags a selection rectangle, and panning never fights one.
   navGest/panDrag are set by view.js's capture handler — which runs
   BEFORE this one on the same event — so the check reliably wins. */
const MARQUEE_DZ = 8; /* px of movement before the draft shows */

viewport.addEventListener(
  "pointerdown",
  (e) => {
    if (mode !== "select") return;
    if (navGest || panDrag || spaceDown) return; /* a pan owns this gesture */
    if (
      ...
      return;
    if (e.target !== viewport) return; /* objects stay interactive */
    e.preventDefault();
    e.stopPropagation();
    try {
    ...
    /* first movement past the deadzone: build and show the draft */
    const arm = () => {
      started = true;
      if (selStyle === "lasso") {
        if (!lassoSvg) {
    ...
    const mv = (e2) => {
      ...
      if (!started) {
        if (Math.hypot(cx - sx, cy - sy) < MARQUEE_DZ) return;
        arm();
      }
      if (selStyle === "lasso") {
        scr.push([cx, cy]);
    ...
    const up = () => {
      ...
      clearSel();
      if (!started) {
        /* a tap (under the deadzone, or no movement at all): select
           whatever sits at the point */
