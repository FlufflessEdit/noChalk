...
function drawDot(ctx, s) {
  ...
  ctx.fillStyle = s.c;
  ctx.beginPath();
  /* a one-point stroke has no segment to average — segW(s, 1) would read
     pts[1], which does not exist. Fall back to the preset width. */
  const r = s.t === "hl" ? s.w || 13 : s.pts.length > 1 ? segW(s, 1) : s.w;
  ctx.arc(p[0], p[1], Math.max(1, r / 2), 0, 7);
  ctx.fill();
  ctx.globalAlpha = 1;
}
...
function syncShapeBtns() {
  const map = {
    ...
    tri: ["shapeTriBtn", "#i-tri", "#i-rect-f"],
  };
  for (const k in map) {
...
inkC.addEventListener("pointerdown", (e) => {
  ...
  if (shapeArm) {
    ...
    if (e.pointerType === "touch" && typeof touchDrew !== "undefined") touchDrew = true;
    return;
  }
...
function endStroke() {
  if (drawing) {
    ...
    const en = { k: "add", strokes: [drawing], plane };
    pushUndo(
      () => revertEntry(en),
      () => applyEntry(en),
    );
  ...
  commitEraseUndo(); /* the eraser drag ends here — its coalesced entry goes on the stack once */
  erasing = false;
}
...
/* ---- eraser undo: ONE coalesced entry per eraser drag ----
   Per-sample entries would flood the undo stack (its 80-deep cap would be
   eaten by a single drag) and Ctrl+Z would restore one tiny chunk at a
   time. Instead, the samples of one drag accumulate into eraseUndo and the
   entry is pushed once, when the drag ends (endStroke → commitEraseUndo). */
let eraseUndo = null; /* { plane: { removed: [], added: [] } } */
function eraseAt(x, y) {
  ...
  for (const plane of ["ink", "shapeInk"]) {
    ...
    let rem, add;
    if (er.k === "line") {
      rem = arr.filter((s) => strokeHit(s, x, y, 11 / VP.z));
      add = [];
    } else {
      const split = eraseSplit(arr, x, y, er.r / VP.z);
      rem = split.rem;
      add = split.add;
    }
    ...
    if (!eraseUndo) eraseUndo = {};
    const e = (eraseUndo[plane] = eraseUndo[plane] || { removed: [], added: [] });
    /* a fragment erased or split further by a later sample of the same drag
       drops out of `added` first, so undo AND redo both land on the exact
       end-of-drag state */
    e.added = e.added.filter((s) => !rem.includes(s));
    e.added.push(...add);
    e.removed.push(...rem);
    did = true;
  }
...
function commitEraseUndo() {
  if (!eraseUndo) return;
  const entries = Object.entries(eraseUndo)
    .map(([plane, e]) => ({ k: "rep", removed: e.removed, added: e.added, plane }))
    .filter((en) => en.removed.length || en.added.length);
  eraseUndo = null;
  if (!entries.length) return;
  pushUndo(
    () => entries.forEach((en) => revertEntry(en)),
    () => entries.forEach((en) => applyEntry(en)),
  );
}
