...
plane.addEventListener("paste", (e) => {
  ...
  /* pasted HTML SOURCE (the plain text itself is markup) → live HTML box,
     not inline soup inside the text box */
...
plane.addEventListener("input", () => {
  ...
  autoListFromInput();
  if (cur.pg && (cur.pg.shapes || []).some((s) => s.anchor)) reflowAnchoredCovers();
});
...
/* ---- auto lists: "1. ", "3. " and "- " at the start of a row ---- */
function autoListFromInput() {
  const sel = getSelection();
  if (!sel || !sel.isCollapsed || !sel.rangeCount) return;
  const n = sel.anchorNode;
  if (!n || n.nodeType !== 3) return;
  const pe = n.parentElement;
  if (!pe || !plane.contains(pe)) return;
  if (pe.closest("li")) return; /* already in a list */
  const cont = pe.closest(".cont");
  if (!cont) return;
  const block = pe.closest("p,div,h1,h2,h3,blockquote,pre") || cont;
  const r = document.createRange();
  r.selectNodeContents(block);
  r.setEnd(n, sel.anchorOffset);
  const before = r.toString();
  const m = /^(\d{1,3})[.)] $/.exec(before);
  if (m) {
    r.deleteContents();
    ...
    sel.removeAllRanges();
    sel.addRange(r3);
    startOrderedList(parseInt(m[1], 10));
    return;
  }
  if (/^[-*] $/.test(before)) {
    r.deleteContents();
...
async function startOrderedList(num) {
  ...
  if (!ol) return;
  /* a numbered list above, same indent, ending at num-1? */
  let prev = ol.previousElementSibling;
  while (prev && !/^(OL|UL)$/.test(prev.tagName)) prev = prev.previousElementSibling;
  if (prev && prev.tagName === "OL" && prev.parentElement === ol.parentElement) {
    const lastNum = (prev.getAttribute("start") ? parseInt(prev.getAttribute("start"), 10) : 1) + prev.children.length - 1;
    if (lastNum === num - 1) {
      const r = await dialog({
        title: "Numbered list",
        msg: "The list above ended at " + lastNum + ". Continue it?",
        ok: "Continue",
        extra: "Start new at " + num,
        extraValue: "new",
      });
      if (r === "new") {
      ...
      if (r) {
        prev.append(li); /* one flowing list — renumbers itself */
        return;
      }
    }
  }
...
/* ---- edit the HTML of an existing run-box ---- */
function editRunHtml(rec, w) {
  const n = el("div", "pop-preset");
  n.style.minWidth = "680px";
  n.style.maxWidth = "min(96vw, 680px)";
  n.append(el("div", "pop-h", "Edit HTML"));
  const ta = el("textarea", "html-tool-in html-tool-edit");
  ta.style.width = "100%";
  ta.style.height = "60vh";
  ta.value = rec.runHtml || "";
  ta.spellcheck = false;
  n.append(ta);
  const row = el("div", "dlg-b");
  const run = el("button", "btn go", "Apply");
  row.append(run);
  n.append(row);
  run.addEventListener("click", () => {
    rec.runHtml = ta.value;
    closePop();
    const box = w || mediaplane.querySelector('.tbox[data-cid="' + rec.id + '"]') || plane.querySelector('.tbox[data-cid="' + rec.id + '"]');
    if (box) {
      box.dataset.live = "";
      const st = box.querySelector(".tbox-ro");
      if (st) st.innerHTML = "";
      makeRunHtmlLive(box, rec);
    }
    queueSave();
  });
