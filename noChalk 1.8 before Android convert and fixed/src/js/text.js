...
let txColor = (state && state.ui && state.ui.txColor) || "#e2e2e2";
(function syncColDot() {
  const d = document.getElementById("colDot");
  if (d) d.style.background = txColor;
})();
function setTypingColor(c) {
  if (!focusContent()) return;
  const sel = getSelection();
  if (sel && !sel.isCollapsed && sel.rangeCount) {
    exec("foreColor", c); /* a selection: reformat it */
    return;
  }
  if (!sel || !sel.isCollapsed || !sel.rangeCount) return;
  /* caret: a zero-width marker span carries the colour for what you type
     next — same technique as the font-size field */
  const r = sel.getRangeAt(0);
  const span = document.createElement("span");
  span.style.color = c;
  span.appendChild(document.createTextNode("\u200B"));
...
(function wireColBtn() {
  ...
  holdEdit(
    b,
    () => setTypingColor(txColor) /* click: apply to the selection / set for typing */,
    () => {
      const node = swatchNode(TXCOLORS, txColor, (c) => {
        txColor = c;
        state.ui.txColor = c;
      ...
      const cust = node.querySelector(".sw-cust input");
      if (cust)
        cust.addEventListener("change", () => {
          txColor = cust.value;
          $("#colDot").style.background = txColor;
          setTypingColor(txColor);
        });
      openPop(b, node);
    },
  );
