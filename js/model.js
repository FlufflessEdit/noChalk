...
function renderPages() {
  ...
  cur.sec.pages.forEach((st) => {
    ...
    const bDup = el("button", "pi", '<svg class="ic"><use href="#i-copy"/></svg>');
    bDup.title = "Duplicate page";
...
async function duplicatePage(stub) {
  await savePageNow();
  const my = ++openSeq;
  let src = cur.pg && cur.pg.id === stub.id ? cur.pg : await idbGet("pages", stub.id);
  if (my !== openSeq) return;
  if (!src) src = stub;
